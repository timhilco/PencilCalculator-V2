...
func main() {
	...
	// display info.
	shell.Println("Pencil Calculator")
	shell.AddCmd(&ishell.Cmd{
		Name: "Calculator",
		Help: "input in multiple lines",
		Func: func(c *ishell.Context) {
			c.Println("Input expression and end with semicolon '#'.")
			lines := c.ReadMultiLines("#")
			c.Println("---------------Program Results-------------------------------------")
			results := evaluate(lines)
			c.Println(results)
			results2 := evaluate(lines)
...
func example() {
	...
	// shell.SetMultiChoicePrompt(" >>"," - ")help
